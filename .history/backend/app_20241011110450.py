...
def manage_flashcards():
    ...
    if request.method == 'POST':
        ...
        # ตรวจสอบว่ามีการส่งค่า term และ meaning หรือไม่
        if term and meaning:
            # Set incorrect_count เป็น 0 โดยเริ่มต้นเมื่อเพิ่ม flashcard ใหม่
            new_flashcard = {'term': term, 'meaning': meaning, 'incorrect_count': 0}

            # ตรวจสอบว่าข้อมูลซ้ำหรือไม่ด้วย uniqueness_testing
            ...
            print("Term or meaning is missing!")
...
def randomize_flashcards():
    global flashcards
    if flashcards:  # ตรวจสอบว่ามี flashcards ในระบบหรือไม่
        flashcards = random_flashcards(flashcards)
    else:
        print("No flashcards to randomize!")
...
def start_review():
    global flashcards
    if flashcards:  # ตรวจสอบว่ามี flashcards สำหรับเริ่มทบทวนหรือไม่
        return redirect(url_for('review_flashcard', index=0))
    else:
        print("No flashcards available for review!")
        return redirect(url_for('manage_flashcards'))
