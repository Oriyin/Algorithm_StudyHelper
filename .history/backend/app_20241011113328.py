...
def manage_flashcards():
    ...
    if request.method == 'POST':
        ...
        # Create the new flashcard with incorrect_count initialized to 0
...
# Route to start flashcard practice
@app.route('/start_practice')
def start_practice():
    global flashcards
    # Randomize flashcards before starting the practice
    flashcards = random_flashcards(flashcards)
    return redirect(url_for('review_flashcard', index=0))

# Route to practice again after completion
@app.route('/practice_again')
def practice_again():
    global flashcards
    # Randomize flashcards before starting the practice again
    flashcards = random_flashcards(flashcards)
    return redirect(url_for('review_flashcard', index=0))
...
def practice_completed():
    correct_flashcards = [f for f in flashcards if f['incorrect_count'] == 0]
    incorrect_flashcards = [f for f in flashcards if f['incorrect_count'] > 0]
