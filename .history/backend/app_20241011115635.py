...
# Route for reviewing flashcards one by one during practice
...
# Route for completing practice session and showing results
@app.route('/practice_completed')
def practice_completed():
    global flashcards
    # Set a threshold to classify flashcards (low incorrect_count threshold)
    low_threshold = 1  # Adjust this threshold as needed

    # Use the divide and conquer algorithm to group flashcards based on incorrect_count
    grouped_flashcards = divide_and_conquer_flashcards(flashcards, low_threshold)

    # Flashcards with zero incorrect answers are considered correct
    ...
    # Render the results page after practice
...
# Route to practice again (randomizes flashcards and restarts the session)
@app.route('/practice_again', methods=['GET'])
def practice_again():
    global flashcards
    # Randomize flashcards before starting practice again
    flashcards = random_flashcards(flashcards)
    return redirect(url_for('practice_flashcard', index=0))

@app.route('/sort_flashcards_by_difficulty', methods=['POST'])
def sort_flashcards_by_difficulty():
    global flashcards
    flashcards = counting_sort(flashcards, 'difficulty')  # ใช้ Counting Sort จัดเรียงตาม difficulty
