...
# Simple Randomization using for loop and random.randrange()
def simple_randomize_flashcards(flashcards):
    randomized_flashcards = []  # List to store the shuffled flashcards
    available_indices = list(range(len(flashcards)))  # Track available indices

    for _ in range(len(flashcards)):  # Loop through the number of flashcards
        random_index = random.randrange(0, len(available_indices))  # Pick a random index
        selected_flashcard = flashcards[available_indices[random_index]]  # Get the flashcard at that index
        randomized_flashcards.append(selected_flashcard)  # Add it to the randomized list
        available_indices.pop(random_index)  # Remove the used index
