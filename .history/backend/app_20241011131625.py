...
def practice_again():
    ...
    # ตรวจสอบว่าอยู่ในโหมด Easy to Hard หรือไม่ ถ้าใช่ ใช้ counting_sortflash
    if request.args.get('sort_mode') == 'easy_to_hard':
        flashcards = counting_sortflash(flashcards, 'difficulty')  # เรียงลำดับจากง่ายไปยาก
    else:
